...
def compute_avg_conll_f1(gold_path, predictions, official_stdout=False):
    conll_results = evaluate_conll(
        gold_path, predictions, official_stdout
    )
    return sum(results["f"] for results in conll_results.values()) / len(conll_results)